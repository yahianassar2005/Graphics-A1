...
COLORREF getDrawColor() {
    static const COLORREF pal[] = {
        RGB(255, 255, 255), RGB(255, 80, 80), RGB(80, 255, 120), RGB(90, 180, 255),
        RGB(255, 220, 60), RGB(255, 120, 255), RGB(200, 200, 200), RGB(255, 160, 40),
    };
...
void changeMouseShape() {
    ...
    curIx = (curIx + 1) % 3;
    HCURSOR h = LoadCursor(nullptr, curIx == 0 ? IDC_ARROW : curIx == 1 ? IDC_CROSS : IDC_HAND);
...
void ConvexFill(const vector<POINT>& poly, vector<FillEdgeTableEntry>& table, HDC hdc, COLORREF c) {
    ...
    for (int y = 0; y < (int)table.size(); y++) {
        if (table[y].xmin <= table[y].xmax) {
            for (int x = table[y].xmin; x <= table[y].xmax; x++) SetPixel(hdc, x, y, c);
        }
    }
}
...
void fillC(int xc, int yc, int r, int q, HDC hdc, COLORREF c) {
    for (int y = -r; y <= r; y++) {
        for (int x = -r; x <= r; x++) {
            if (x * x + y * y <= r * r) {
                bool test = false;
                if (q == 1 && x >= 0 && y >= 0) test = true;
                if (q == 2 && x <= 0 && y >= 0) test = true;
                if (q == 3 && x <= 0 && y <= 0) test = true;
                if (q == 4 && x >= 0 && y <= 0) test = true;
                if (test) SetPixel(hdc, xc + x, yc + y, c);
            }
        }
    }
}
...
void fillS(int xmin, int xmax, int ymin, int ymax, HDC hdc, COLORREF c) {
    for (double t = 0; t <= 1; t += 0.01) {
        ...
        for (int x = xmin; x <= xmax; x++) SetPixel(hdc, x, y, c);
    }
}
...
void fillR(int xmin, int xmax, int ymin, int ymax, HDC hdc, COLORREF c) {
    for (double t = 0; t <= 1; t += 0.01) {
        ...
        for (int y = ymin; y <= ymax; y++) SetPixel(hdc, x, y, c);
    }
}

void ffR(int x, int y, COLORREF oc, COLORREF nc, HDC hdc) {
    if (GetPixel(hdc, x, y) != oc) return;
    SetPixel(hdc, x, y, nc);
    ffR(x + 1, y, oc, nc, hdc);
    ffR(x - 1, y, oc, nc, hdc);
    ffR(x, y + 1, oc, nc, hdc);
...
void fillP(vector<POINT> v, HDC hdc, COLORREF c) {
    ...
    for (int y = ymin; y <= ymax; y++) {
        ...
        for (int i = 0; i < (int)n.size(); i += 2) {
            if (i + 1 < (int)n.size()) {
                for (int x = n[i]; x <= n[i + 1]; x++) SetPixel(hdc, x, y, c);
            }
        }
    }
}
...
static int fillPolyMaxY(const vector<POINT>& poly) {
    int m = 0;
    for (const auto& p : poly) m = std::max(m, (int)p.y);
    return m;
}
...
static void FillCircleQuarterWithCircles(HDC hdc, int xc, int yc, int r, int q, COLORREF c) {
    for (int rk = 1; rk <= r; rk++) drawCircleMidpoint(hdc, xc, yc, rk, c);
...
static bool cohenSutherlandClip(double xmin, double xmax, double ymin, double ymax,
    ...
    while (true) {
        ...
        double xi = 0, yi = 0;
        bool ok = false;
        if (out & CS_BOTTOM) ok = intersectCSBoundary(x0, y0, x1, y1, CS_BOTTOM, xmin, xmax, ymin, ymax, xi, yi);
        else if (out & CS_TOP) ok = intersectCSBoundary(x0, y0, x1, y1, CS_TOP, xmin, xmax, ymin, ymax, xi, yi);
        else if (out & CS_RIGHT) ok = intersectCSBoundary(x0, y0, x1, y1, CS_RIGHT, xmin, xmax, ymin, ymax, xi, yi);
        else if (out & CS_LEFT) ok = intersectCSBoundary(x0, y0, x1, y1, CS_LEFT, xmin, xmax, ymin, ymax, xi, yi);
        if (!ok)
            return false;
...
static void replayOne(HDC hdc, const string& s) {
    ...
    double tens;
    int rr, gg, bb;
    auto readC = [&](istringstream& is) -> bool { return !!(is >> rr >> gg >> bb); };
    ...
    if (tok == "LINE_DDA" && iss >> x1 >> y1 >> x2 >> y2 && readC(iss))
        ...
        bool ok = true;
        for (int i = 0; i < n; ++i) {
            if (!(iss >> poly[i].x >> poly[i].y)) ok = false;
        }
        if (ok && readC(iss))
        ...
        case 5: {
            ...
            if ((int)pt.size() >= 2) {
                ...
                if (subOption >= 1 && subOption <= 3 && hdc) {
                    string tag = subOption == 1 ? "E_DIR" : subOption == 2 ? "E_POL" : "E_MID";
                    ostringstream oe;
